		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"100%","margin":"0px 0 0","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"minHeight":"100vh","padding":"34px 20px 140px","boxShadow":"0 2px 0px rgba(0,0,0,.1)","top":"94px","textAlign":"center","background":"url(http://codegen.caihongy.cn/20221203/1f4690773fd84e218e0ef7d69885ee30.png) no-repeat left top,url(http://codegen.caihongy.cn/20221203/1057db36d2aa4bd4b375aa0ccb752a40.png) repeat-x left top,#7ca065","display":"block","width":"240px","position":"fixed","right":"0","order":"1"}' >
					<div :style='{"flexWrap":"wrap","margin":"0 0px 10px","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#fff","textAlign":"left","display":"inline-block","width":"100%","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">作业标题</label>
						<el-input v-model="searchForm.zuoyebiaoti" placeholder="作业标题" clearable></el-input>
					</div>
				...
				<el-row :style='{"width":"100%","padding":"0","margin":"20px 0 10px","background":"none","display":"flex"}'>
					<el-button :style='{"border":"0px dotted #a347ea","cursor":"pointer","padding":"0 8px","boxShadow":"4px 4px 0px rgba(0,0,0,.1)","margin":"0 10px 10px 0","color":"#7ca065","minWidth":"68px","outline":"none","borderRadius":"0px","background":"rgba(124,160,101,.2)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('zuoyegonggao','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0px dotted #df2543","cursor":"pointer","padding":"0 8px","boxShadow":"4px 4px 0px rgba(0,0,0,.1)","margin":"0 10px 10px 0","color":"#d67081","minWidth":"68px","outline":"none","borderRadius":"0px","background":"rgba(223,37,67,.2)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('zuoyegonggao','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="zuoyefengmian" width="200" label="作业封面">
						<template slot-scope="scope">
							<div v-if="scope.row.zuoyefengmian">
								<img v-if="scope.row.zuoyefengmian.substring(0,4)=='http'" :src="scope.row.zuoyefengmian.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.zuoyefengmian.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='false' prop="zuoyewenjian" label="作业文件">
						<template slot-scope="scope">
							<el-button v-if="scope.row.zuoyewenjian" type="text" size="small" @click="download($base.url+scope.row.zuoyewenjian)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"0px solid #18cfa5","cursor":"pointer","padding":"0 16px","boxShadow":"2px 2px 0px rgba(100,100,100,.1)","margin":"3px 6px 3px 0","outline":"none","color":"#18cfa5","borderRadius":"4px","background":"rgba(24,207,165,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('zuoyegonggao','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"0px solid #1fbfae","cursor":"pointer","padding":"0 16px","boxShadow":"2px 2px 0px rgba(100,100,100,.1)","margin":"3px 6px 3px 0","outline":"none","color":"#4d9be3","borderRadius":"4px","background":"rgba(77,155,227,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('zuoyegonggao','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"0px solid #dc333b","cursor":"pointer","padding":"0 16px","boxShadow":"2px 2px 0px rgba(100,100,100,.1)","margin":"3px 6px 3px 0","outline":"none","color":"#dc333b","borderRadius":"4px","background":"rgba(220,51,59,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('zuoyegonggao','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
